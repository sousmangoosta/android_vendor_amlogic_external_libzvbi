...
CONFIG_CLEAN_FILES = zvbi.spec zvbi-0.2.pc
SOURCES =
DIST_SOURCES =
...
zvbi.spec: $(top_builddir)/config.status $(srcdir)/zvbi.spec.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
zvbi-0.2.pc: $(top_builddir)/config.status $(srcdir)/zvbi-0.2.pc.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
distdir: $(DISTFILES)
	...
	*) \
	...
	$(am__remove_distdir)
	mkdir $(distdir)
	$(mkdir_p) $(distdir)/. $(distdir)/daemon $(distdir)/doc $(distdir)/m4 $(distdir)/po
